...
public class FileModel implements Serializable {
	...
	public FileModel(String namespace, String objectId, byte[] content,String uploadFileName) {
		this.namespace = namespace;
		this.objectId = objectId;
		if (uploadFileName != null && !"".equals(uploadFileName.trim())) {
			this.ext = uploadFileName.substring(uploadFileName.lastIndexOf(".")+1);//取不带  .  的后缀名
			InputStream is = null;
			try {
				//this.size = String.valueOf(file.length());
				this.filename = uploadFileName.substring(0, uploadFileName.lastIndexOf("."));
