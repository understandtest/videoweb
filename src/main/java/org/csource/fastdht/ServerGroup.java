...
* @return group info
*/
	public static ServerGroup loadFromFile() throws MyException
	{
			ServerGroup serverGroup;
  		...
  		String[] szServers;
			...
			serverGroup = new ServerGroup();
  		keep_alive = PropertiesUtil.getInt("fastDHT.keep_alive");
  		serverGroup.keep_alive = keep_alive != 0;
  		
  		group_count = PropertiesUtil.getInt("fastDHT.group_count");
  		if (group_count <= 0)
  		{
  		...
  		{
  			szServers = PropertiesUtil.getString("fastDHT.group"+i).split(",");
	  		if (szServers == null || szServers.length == 0)
	  		{
	  			throw new MyException("item \"group" + i + "\" in system.properties not found");
	  		}
